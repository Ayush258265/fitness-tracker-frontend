...
function WorkoutSession({ workout, onComplete }) {
    ...
    const saveWorkoutToBackend = useCallback(async (stats, completedExList) => {
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/workout/history/save`, {
                method: "POST",
    ...
    const handleSkip = async () => {
        if (phase === "exercise") {
            ...
            const halfCalories = Math.floor(originalCalories / 2);  // ← CHANGED
            const newRemaining = remainingCalories - halfCalories;

            console.log(`\n⏭️ SKIPPED EXERCISE: ${current?.exerciseName}`);
            console.log(`   🔥 Original calories: ${originalCalories} kcal → Skipped: ${halfCalories} kcal`);  // ← UPDATED
            console.log(`   📉 Remaining calories: ${newRemaining} kcal (was ${remainingCalories})`);

            setRemainingCalories(newRemaining);

            const skippedExercise = { ...current, caloriesBurned: halfCalories };  // ← CHANGED
